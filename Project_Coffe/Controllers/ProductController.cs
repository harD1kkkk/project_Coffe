...
namespace Project_Coffe.Controllers
{
    ...
    public class ProductController : ControllerBase
    {
        ...
        [Authorize]
        [HttpPut("sort-by-lower-price")]
        public async Task<IActionResult> SortByLowerPrice(decimal price)
        {
            try
            {
                List<Product> products = (await _productService.SortByLowerPrice(price)).ToList();
                if (!products.Any())
                {
                    _logger.LogInformation($"No products found with price containing '{price}'.");
                    return NotFound($"No products found with price containing '{price}'.");
                }
                _logger.LogInformation("Fetched products successfully.");
                return Ok(products);
            }
        ...
        [Authorize]
        [HttpPut("sort-by-higher-price")]
        public async Task<IActionResult> SortByHigherPrice(decimal price)
        {
            try
            {
                List<Product> products = (await _productService.SortByHigherPrice(price)).ToList();
                if (!products.Any())
                {
                    _logger.LogInformation($"No products found with price containing '{price}'.");
                    return NotFound($"No products found with price containing '{price}'.");
